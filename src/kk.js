...
const getAllBranches = async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM fnb_branches');
    res.json(result.rows);
  } catch (err) {
    console.error(err);
    res.status(500).send('Server error');
...
const getBranchById = async (req, res) => {
  const { id } = req.params;
  try {
    const result = await pool.query('SELECT * FROM fnb_branches WHERE id = $1', [id]);
    res.json(result.rows[0]);
  } catch (err) {
    console.error(err);
    res.status(500).send('Server error');
...
const createBranch = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'INSERT INTO fnb_branches (branch_name, branch_code, created_at) VALUES ($1, $2, NOW()) RETURNING *',
      [branch_name, branch_code]
    );
    res.json(result.rows[0]);
  } catch (err) {
...
const deleteBranch = async (req, res) => {
  const { id } = req.params;
  try {
    await pool.query('DELETE FROM fnb_branches WHERE id = $1', [id]);
    res.sendStatus(204);
  } catch (err) {
    console.error(err);
    res.status(500).send('Server error');
